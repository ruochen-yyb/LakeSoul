...
- `--transfer.claimed-by`：worker 标识，例如 `flink-worker-1`，会作为新接口的 `workerId`
...
- `--transfer.done-retry-max-attempts`：成功回调最大重试次数，默认 `3`
- `--transfer.done-retry-sleep-ms`：成功回调重试间隔，默认 `5000`
...
flink run -c org.apache.flink.lakesoul.entry.transfer.UzsAutoTransfer lakesoul-flink.jar \
  ...
  --transfer.sql-timeout-ms 1800000 \
  --transfer.lease-ms 1860000 \
