...
- 触发源：轮询 `getCompactionTask` 接口（不再依赖 PG `LISTEN/NOTIFY`）
- 行为：领取任务后执行 `LakeSoulTable.uzsFullPartitionCompaction`；成功回调 `setTaskDone`，失败回调 `setTaskErr`
- 参数：
  - `--poll.base.url`：任务服务地址，默认 `http://127.0.0.1:8080`
  - `--claimed.by`：worker 标识，默认 `spark-worker`
